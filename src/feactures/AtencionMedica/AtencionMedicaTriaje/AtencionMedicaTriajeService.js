...
/*
// AtencionMedicaTriajeService.js
import { getCatalogoInit } from '../common/catalogoService'; // Ajusta la ruta relativa según tu estructura
...
export const AtencionMedicaTriajeService = {
  ...
  // 🛠️ Se añade idEntidad como parámetro para pasarlo al servicio de caché
  buscarEnCatalogo: async (textoBusqueda, signosVisiblesActuales = [], idEntidad = null) => {
    if (!textoBusqueda || textoBusqueda.trim() === '') return [];
    ...
    try {
      // 🚀 OPTIMIZACIÓN CON CACHÉ: Obtenemos los catálogos en memoria RAM usando la entidad correcta
      const catalogoGlobal = await getCatalogoInit(idEntidad);      
      
      // Extraemos la sección correspondiente a triaje/signos vitales del catálogo global
      const listaTriajeMaster = catalogoGlobal.triaje || catalogoGlobal.signosVitales || [];

      return listaTriajeMaster
        .filter(item => item.nombre.toLowerCase().includes(query))
        .map(item => ({ 
          id: item.id, 
          label: item.nombre, 
      ...
      console.error("⚠️ Error al buscar en el catálogo en caché, usando respaldo local:", error);
      return [];
    }
  }
};
