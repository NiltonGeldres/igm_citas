...
export const AtencionMedicaTriajeService = {
    ...
    obtenerTriajeInicial: (datosGlobalesParam = null) => {
        // 1. Usa el parámetro si se recibe; de lo contrario lee el caché global
        let catalogo = obtenerCatalogoTriajesDesdeCache()  ;
    ...
    /**
     * Carga para Atención Registrada: Muestra lo devuelto por la API de la atención
     */
    obtenerTriajeRegistrado: (dataAtencion = {}) => {
      let triajesApi = Array.isArray(dataAtencion) 
        ? dataAtencion 
        : (dataAtencion?.triajes || dataAtencion?.triaje || []);

      if (triajesApi.length === 0 && !ES_PRODUCCION) {
        triajesApi = MOCK_TRIAJE_ATENCION_REGISTRADA;
      }

      return AtencionMedicaTriajeMapper.transformarApiALFront(triajesApi);
    },
